...
// Inline SVG constants, moved verbatim from the retired select-view (their
// design history lives in the git record of that file): a PAPER cup, not a
// mug — a mug on its side reads as a handbag at this size — and the spilled
// pose is the same two paths rotated over a puddle. Steam is static: the
// GDD's no-list bans ambient motion.
const PAPER_CUP = `
...
export class CalendarView {
  constructor(root, { onPick, onBack }) {
    root.innerHTML = `
      <div class="select-head">
        ...
        <p class="select-count">Past Pours</p>
      </div>
      <div class="pours-nav">
  ...
  buildSquare(day) {
    ...
    if (day.result) {
      ...
      if ((day.result.hintsUsed ?? 0) > 0) cup.classList.add('is-hinted');
      cup.innerHTML = day.result.status === 'won' ? CUP_STEAMING : CUP_SPILLED;
      square.appendChild(cup);
    }
