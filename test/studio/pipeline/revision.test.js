...
test('running a revision re-runs from the entry stage forward and nothing before it', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ['04-board-builder', '05-analogy-validator', '06-adversarial-solver', '07-test-player', '08-style-guide', '09-glossary-author', '10-definitions-author'],
    );
  } finally {
    cleanup();
  }
...
test('the evaluators stay blind — they judge the board, not the request', async () => {
  ...
  try {
    ...
    for (const stageId of ['05-analogy-validator', '06-adversarial-solver', '07-test-player', '08-style-guide', '09-glossary-author', '10-definitions-author']) {
